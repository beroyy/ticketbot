 ...
 */
export async function assignDiscordRole(member: GuildMember, roleId: string): Promise<boolean> {
  try {
 ...
 */
export async function removeDiscordRole(member: GuildMember, roleId: string): Promise<boolean> {
  try {
    const role = await member.guild.roles.fetch(roleId);
    if (!role) {
      container.logger.warn(`Discord role ${roleId} not found in guild ${member.guild.id}`);
 ...
 */
export async function syncTeamRoleToDiscord(
  teamRole: Pick<any, "discordRoleId" | "name">,
  userId: string,
  guild: Guild,
  action: "add" | "remove"
): Promise<boolean> {
  if (!teamRole.discordRoleId) {
    container.logger.debug(`Role ${teamRole.name} has no Discord role ID`);
    return false;
  }
  ...
  try {
    ...
    if (action === "add") {
      return await assignDiscordRole(member, roleId);
    } else {
      return await removeDiscordRole(member, roleId);
    }
  } catch (error) {
 ...
 */
export async function syncMultipleRolesToDiscord(
  teamRoles: Array<Pick<any, "discordRoleId" | "name">>,
  userId: string,
  guild: Guild,
  action: "add" | "remove"
  ...
  for (const role of teamRoles) {
    const success = await syncTeamRoleToDiscord(role, userId, guild, action);
    if (success) {
 ...
 */
export function formatRolePermissions(roleName: string): string {
