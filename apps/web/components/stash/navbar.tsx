...
import { useSession, signIn, signOut } from "@ticketsbot/auth/client";
...
import { usePermissions, PermissionFlags } from "@/contexts/permission-context";
...
const navItems: NavItem[] = [
  ...
  },
  {
    href: "/import",
    label: "Import",
    permission: PermissionFlags.TICKET_EXPORT,
    requiresGuild: true,
  },
  {
    href: "/blacklist",
...
export function Navbar() {
  ...
  const visibleNavItems = useMemo(() => {
    return navItems.filter((item) => {
      ...
      if (item.permission && selectedGuildId) {
        return hasPermission(selectedGuildId, item.permission);
      }

      // Check multiple permissions (OR condition - user needs at least one)
      if (item.permissions && selectedGuildId) {
        return item.permissions.some((perm) => hasPermission(selectedGuildId, perm));
      }

      return true;
    });
  }, [selectedGuildId, hasPermission]);
